...
class TestBound(unittest.TestCase):
    ...
    def test_is_satisfied_by(self):
        for _ in range(1000):
            ...
            self.assertTrue(
                d.is_satisfied_by(Version.from_str(f"{major}.{minor}.{patch + 1}"))
            )
            self.assertTrue(
                d.is_satisfied_by(Version.from_str(f"{major}.{minor + 1}.{patch}"))
            )
            self.assertTrue(
                d.is_satisfied_by(Version.from_str(f"{major + 1}.{minor}.{patch}"))
            )
            self.assertFalse(
                d.is_satisfied_by(Version.from_str(f"{major}.{minor}.{patch - 1}"))
            )
